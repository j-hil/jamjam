...
def test_has_instance() -> None:
    ...
    U: TypeAlias = int | str
    assert Check[U].has_instance(1)
    assert Check[U].has_instance("")
    assert not Check[U].has_instance([])

    V: TypeAlias = Union[str, list, tuple]  # noqa: UP007
    assert Check[V].has_instance([])
